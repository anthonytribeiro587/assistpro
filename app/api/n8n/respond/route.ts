...
function providedSecret(request: NextRequest) {
  return (
    request.headers.get('x-assistpro-n8n-secret') ||
...
export async function GET() {
  return NextResponse.json({
    ok: true,
    service: 'AssistPro n8n response callback',
...
export async function POST(request: NextRequest) {
  ...
  if (!expectedSecret) {
    return NextResponse.json({ ok: false, error: 'N8N_WEBHOOK_SECRET não configurado.' }, { status: 503 });
  }
  ...
  try {
    const body = (await request.json()) as N8nResponseBody;
    ...
    if (audioUrl) {
      delivery.audio = await sendAudioMessage({ phone, audioUrl });
    }
    ...
    const message = error instanceof Error ? error.message : 'Não foi possível enviar a resposta do n8n.';
    console.error('AssistPro n8n callback error', message);
